...
BRAND_BLUE = Color(0.05, 0.25, 0.45) 
BRAND_GOLD = Color(0.85, 0.65, 0.13)
...
def fetch_hotel_details_text(hotel, city):
    # Just for address during PDF gen
    model = genai.GenerativeModel('gemini-2.0-flash')
    try:
...
# --- 5. PDF GENERATION (PIXEL PERFECT) ---

def draw_seal(c, x, y):
    c.saveState()
    c.setStrokeColor(BRAND_BLUE); c.setFillColor(BRAND_BLUE); c.setFillAlpha(0.9); c.setLineWidth(1.5)
    c.circle(x+40, y+40, 40, stroke=1, fill=0) # Outer
    c.setLineWidth(0.5); c.circle(x+40, y+40, 36, stroke=1, fill=0) # Inner
    ...
    def draw_arc_text(text, angle, radius, rotate_offset):
        for i, char in enumerate(text):
            rad = radians(angle - (i * 10))
            tx = (x+40) + radius * cos(rad)
            ty = (y+40) + radius * sin(rad)
            c.saveState()
            c.translate(tx, ty)
            c.rotate((angle - (i * 10)) + rotate_offset)
            c.drawCentredString(0, 0, char)
            c.restoreState()
            
    draw_arc_text("CERTIFIED VOUCHER", 140, 32, -90)
    draw_arc_text("OFFICIAL", 235, 32, 90) # Bottom arc needs different rotation logic, simplified here for top-down consistency
...
def generate_pdf(data, info, imgs, rooms_list):
    ...
    for i, room in enumerate(rooms_list):
        ...
        # HEADER
        try: c.drawImage(LOGO_FILE, left, y-40, 100, 40, mask='auto', preserveAspectRatio=True)
        except: 
            c.setFillColor(BRAND_BLUE); c.setFont("Helvetica-Bold", 20); c.drawString(left, y-30, "ODADUU")
            c.setFont("Helvetica", 10); c.drawString(left, y-42, "Travel DMC")
        ...
        if any(images):
            ...
            for img in images:
                if img: 
                    try: c.drawImage(img, ix, y-90, 170, 90); ix += 175
                    except: pass
            y -= 110
        else: y -= 10
        ...
        tnc = [
            ...
            "9. Taxes: City tax payable at hotel.",
...
if st.button("Generate Vouchers", type="primary"):
    with st.spinner("Generating..."):
        ...
        if rooms:
            ...
            # Fallback for images if manual entry
